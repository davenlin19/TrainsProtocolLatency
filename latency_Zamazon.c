...
void callbackUtoDeliver(address sender, message *mp){
  ...
  if (mp->header.typ == AM_PING) {
	  ...
	  memcpy(&addressOrder, mp->payload, sizeof(address));
	  memcpy(&sendDate, mp->payload + sizeof(address), sizeof(struct timeval));
	  memcpy(&product_id, mp->payload + sizeof(address) + sizeof(struct timeval), sizeof(int));
	  memcpy(&quantity, mp->payload + sizeof(address) + sizeof(struct timeval) + sizeof(int), sizeof(int));

	  // convert time_val sendDate to tm
	  time_t timeSendDate = (time_t) sendDate.tv_sec;
    ...
    if (addrIsMine(addressOrder)) {
		  ...
		  // deliver new order to ZamazonServer
		  char str[100];
		  sprintf(str, "%d#%s#%d#", addrToRank(myAddress), s_time, (latency.tv_sec*1000000+latency.tv_usec));
		  zmq_send (requesterByMe, str, 100, 0);
		  char bufferRecv[2];
		  zmq_recv (requesterByMe, bufferRecv, 2, 0);
		  printf("Receive confirm of newMsg sent by me: %s\n", str);	  
    } else {
		  // deliver new order to ZamazonSync
		  char str[100];
		  sprintf(str, "%d#%d#%d#%s#0#", addrToRank(addressOrder), product_id, quantity, s_time);
...
int * getNewOrder(char * bufRecv, int sizeBuf) {
	int * order = malloc(2*sizeof(int));
	sscanf(bufRecv, "%d#%d", &order[0], &order[1]);
	return order;
}
  ...
  if (rank < broadcasters) {
    ...
    do {
	  // if(rank == 0) { // add if in local 
		  // Wait request from ZamazonServer
		  int sizeBuf = zmq_recv (responder, bufferRecv, 20, 0);
		  int * refMsgRecv = getNewOrder(bufferRecv, sizeBuf);
		  int product_id = refMsgRecv[0];
		  int quantity = refMsgRecv[1];
		  // Reply to ZamazonServer
		  ...
		  gettimeofday(&sendTime, NULL);
			...
			memcpy((mp->payload) + sizeof(address) + sizeof(struct timeval), &product_id, sizeof(int));
			memcpy((mp->payload) + sizeof(address) + sizeof(struct timeval) + sizeof(int), &quantity, sizeof(int));
