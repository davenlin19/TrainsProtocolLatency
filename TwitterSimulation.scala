...
class TwitterSimulation extends Simulation { // 3
  ...
  val httpConf = http // 4
    .baseURL("http://ec2-54-93-88-130.eu-central-1.compute.amazonaws.com:8080") // 5
...
object Tweet {
  ...
  val tweet = repeat(100, "n") {
		exec(http("add_tweet")
		.post("/TwitterServer/REST/WebService/AddTweet"))
    	.pause(1 milliseconds)
		.exec(http("get_tweet")
		.get("/TwitterServer/REST/WebService/GetTweets"))
    	.pause(1 milliseconds)
  }
}
